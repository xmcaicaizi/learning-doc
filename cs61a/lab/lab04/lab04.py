...
def buy(required_fruits, prices, total_amount):
    """打印所有购买每种水果的方式，使得总价格等于 total_amount。
    ...
    def add(fruits, amount, cart):
        # 基本情况：如果没有剩余的水果且金额正好为 0，打印当前购物车的内容
        if not fruits and amount == 0:
            print(cart)
        # 递归情况：如果还有水果需要处理且剩余金额大于 0
        elif fruits and amount > 0:
            fruit = fruits[0]  # 取第一个水果
            price = prices[fruit]  # 获取该水果的价格
            # 尝试购买该水果的 1 个或多个
            for k in range(1, amount // price + 1):
                # 递归调用，将剩余的水果和减去购买该水果的金额传递下去
...
def display(fruit, count):
    ...
    '[1 apple]'
    """
    assert count >= 1 and fruit[-1] == 's'  # 确保数量至少为 1，并且水果名称以 's' 结尾（复数形式）
    # 如果数量为 1，移除水果名称中的 's'
    if count == 1:
        fruit = fruit[:-1]  # 移除复数形式的 's'
    # 返回格式化后的字符串
